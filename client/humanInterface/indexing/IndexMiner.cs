...
namespace indexing
{
    public static class IndexMiner
    {
        // properties
        ...
        // search methods
        public static IEnumerable<IndexableEntity> GetAllIndexRecords()
        {
            // validate search index
            if (!System.IO.Directory.EnumerateFiles(_luceneDir).Any()) return new List<IndexableEntity>();
        ...
        public static IEnumerable<IndexableEntity> Search(string input, string fieldName = "")
        {
            if (string.IsNullOrWhiteSpace(input)) return new List<IndexableEntity>();
        ...
        public static IEnumerable<IndexableEntity> SearchDefault(string input, string fieldName = "")
        {
            return string.IsNullOrWhiteSpace(input) ? new List<IndexableEntity>() : _search(input, fieldName);
        }

        public static IEnumerable<IndexableEntity> SearchByMultipleFields(Dictionary<string, string> searchTerms)
        {
            //return string.IsNullOrWhiteSpace(input) ? new List<IndexableEntity>() : _search(input, fieldName);
            return _searchMultiField(searchTerms);
        }

        private static IEnumerable<IndexableEntity> _searchMultiField(Dictionary<string, string> searchTerms)
        {
            // validation
            if (searchTerms == null) return new List<IndexableEntity>();
        ...
        {
            // validation
            if (string.IsNullOrWhiteSpace(searchQuery.Replace("*", string.Empty).Replace("?", string.Empty))) return new List<IndexableEntity>();
        ...
        private static IEnumerable<IndexableEntity> _mapLuceneToDataList(IEnumerable<Document> hits)
        {
            return hits.Select(_mapLuceneDocumentToData).ToList();
        }
        private static IEnumerable<IndexableEntity> _mapLuceneToDataList(IEnumerable<ScoreDoc> hits, IndexSearcher searcher)
        {
            // v 2.9.4: use 'hit.doc'
            // v 3.0.3: use 'hit.Doc'
            return hits.Select(hit => _mapLuceneDocumentToData(searcher.Doc(hit.Doc))).ToList();
        }
        private static IndexableEntity _mapLuceneDocumentToData(Document doc)
        {
            return new IndexableEntity
            {
                Id = Convert.ToInt32(doc.Get("Id")),
                SubtitleTextId = Convert.ToInt32(doc.Get("TitleId"))
            };
        }
        ...
        {
            AddUpdateLuceneIndex(new List<IndexableEntity> { sampleData });
        }

        public static void AddUpdateLuceneIndex(IEnumerable<IndexableEntity> sampleDatas)
        {
            // init lucene
            var analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_30);
